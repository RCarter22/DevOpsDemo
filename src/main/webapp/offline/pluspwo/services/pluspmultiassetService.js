angular.module('emm').factory('pluspmultiassetService', function(){
	var options = {
			defaultPageSize: 20,
			viewName : null,
			userInfo:null
	...
	PUBLIC.showDetail  = function(asset){
		...
		EMMServer.DB.Select()
			...
			.submit("offline/pluspwo/multiasset.htm", true);
	}
...
//						if(asset.PROGRESS && (asset.PROGRESS == true || asset.PROGRESS == 'true' || asset.PROGRESS == 1 || asset.PROGRESS == '1'))
//							asset.PROGRESS = '1';
//						else
//							asset.PROGRESS = '0';
						...
						.then(function(result){
							...
							EMMServer.DB.Select()
								...
								.go("offline/pluspwo/multiasset.htm");
						});						
			...
			markProgress : function(asset){			
				...
				.then(function(result){
					...
					EMMServer.DB.Select()
						.addMessage(getText('RECORDSAVED', null, 'Record Saved'))
						.go("offline/pluspwo/multiassetlist.htm");
				});			
			}			
		};
