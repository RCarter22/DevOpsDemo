angular.module('emm').factory('wotrackService', function($filter, domainService, doclinksService, labtransService, classificationService, worklogService, inspectorService){
	...
	PUBLIC.setStartCenterApplicationName('WOTRACK');
	...
	PUBLIC.toAdvancedSearch = function(){		
		...
		.submit('offline/wotrack/advancedsearch.htm', true);
	};
	...
	PUBLIC.toList = function(searchValue, goBack, query){
		var viewName = 'offline/wotrack/list.htm';
		if(!query)
			query = "SELECT * FROM WORKORDER WHERE ISTASK='0'";
	...
	PUBLIC.createNewTask = function(workorder){
		...
		EMMServer.DB.Select()
			...
			.addQuery("EXTRA", extraSql)
			.submit("offline/wotrack/task.htm", true);
	};
	...
	PUBLIC.actions = {
		...
		toTaskList: function(wo){
			...
			EMMServer.DB.Select()
				...
				.submit("offline/wotrack/tasklist.htm", true);
		...
		reportDowntime : function(workorder){			
			...
			EMMServer.DB.Select()
				...
				.submit("offline/wotrack/downtime.htm", true);
		},	
		...
		toMoveAsset: function(workorder){
			...
			EMMServer.DB.Select()
				...
				.submit("offline/wotrack/moveasset.htm", true);	
		},
		...
		toParent: function(wo){
			wo.session.remove();
			EMMServer.DB.Select()
				.go("offline/wotrack/wotrack.htm");
		},		
		toMultiAssets: function(wo){
			...
			EMMServer.DB.Select()
				...
				.submit("offline/wotrack/multiassetlist.htm", true);
		},
		...
		toFailureReporting: function(wo){
			...
			EMMServer.DB.Select()
				...
				.submit("offline/wotrack/failurereporting.htm", true);
		},
