angular.module('emm').factory('plustlrpService', function(domainService) {
	var options = {
			defaultPageSize: 20,
			viewName : null,
			queryName : null,
	...
	PUBLIC.enterByLabor = function(laborInfo) {
		...
		EMMServer.DB.Select()
			...
			.submit('offline/plustlrp/enterbylabor.htm', true);
	};
	...
	PUBLIC.toAdvancedSearch = function() {
		...
		EMMServer.DB.Select()
			...
			.submit('offline/plustlrp/advancedsearch.htm', true);
	};
	
	PUBLIC.doAdvancedSearch = function(labtrans) {
		var viewName = 'offline/plustlrp/list.htm';
	...
	PUBLIC.showDetail = function(labtrans, message) {
		...
		EMMServer.DB.Select()
			...
			.addMessage(message)
			.submit("offline/plustlrp/labrep.htm",true);
	};
