...
import psdi.plust.app.asset.PlusTAssetRemote;
...
public class PlusTAssetAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	
	public static final String OWNERMBO = "ASSET";
	
	public static final String APPNAME = "PLUSTASSET";
	...
	@Action(value="list",
			results={
				...
				@Result(name="details", location="view.action", type="redirect",params={"id","${id}"}),
	...
	public String list() {
		try{
			...
			//if the result set is exactly 1 record
			//automatically advance to the details page
			if(mboList != null && mboList.size() == 1){
	...
	public String advancedsearch() {
		...
		public String doadvancedsearch() {
		      try{
				  ...
				  String [] relationshipAttrs = {"PLUSTDEFASSETALIAS.ALIAS","PLUSTLICENSES.LICENSENUM","ASSETUSER.PERSONID"};
	...
	public String createWorkorder() {
		try{
			...
			mbo = (MboRemote)((PlusTAssetRemote)mbo).createWorkorder(null);
	...
	public String createServiceRequest() {
		try{
			...
			mbo = (MboRemote)((PlusTAssetRemote)mbo).createServiceRequest(null);
	...
	public String saveSelectedSpareParts() {
		try {			
			...
			this.setSessionObject("SPAREPARTS", null);
			PlusTAssetRemote assetRemote = (PlusTAssetRemote)mbo;
