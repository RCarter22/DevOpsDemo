 ...
 * All rights reserved.
 ******************************************************************************/
package com.interprosoft.ezmaxmobile.plustassetalias.action;
...
@Component
@Scope("prototype")
@Namespace("/plustassetalias")
@ResultPath(value="/")
@ParentPackage(value="mydefault")
@Results({@Result(name="error", location="/error/error.jsp")})
public class PlusTAssetAliasAction extends BaseAction {
	
	private static final long serialVersionUID = 1L;

	public static final String OWNERMBO = "PLUSTASSETALIAS";
	...
	public String execute() {
		try {
			...
			populateMboListByRelationship(currentMbo, "PLUSTNONIDENTITYALIASES");
	...
	@Action(value="view",
			results={
				@Result(name="success", location="plustassetalias.jsp"),
				@Result(name="error", location="plustassetalias.jsp")
			}
		)
	public String view() {
		try{
	...
	public String delete() {
		try {
			...
			MboRemote mboRemote = this.simpleService.findById(mbo.getMboSet("PLUSTNONIDENTITYALIASES"), id);
			id = tmpId;
			mboRemote.delete();
		} catch (Exception e){
			try{
	...
	public String setOtherRecordsAsNonDefault(){
		try {						
			...
			if(mbo.isModified("ISDEFAULT") && mbo.getBoolean("ISDEFAULT")){
				MboSetRemote aliasSet = mbo.getOwner().getMboSet("PLUSTNONIDENTITYALIASES");
				...
				while(aliasMbo != null){
					...
					System.out.println(aliasMbo.getString("DESCRIPTION") + " " + aliasMbo.getString("ISDEFAULT"));
