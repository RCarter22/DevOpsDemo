...
public class PlusTWOAction  extends BaseAction {

	private static final long serialVersionUID = 1L;
	
	public static final String APPNAME = "PLUSTWO";	
	...
	public String doadvancedsearch() {
	      try{
	            ...
	            if(mboSet != null)
				  ...
				  //any search values that come from relationships must be manually added
				  //.getQbe() method no longer returns QBE's based on Maximo relationships
				  String [] relationshipAttrs = {"PLUSTDEFASSETALIAS.ALIAS"};
				  String [] relationshipVals;
				  relationshipVals = mbo.getThisMboSet().getQbe(relationshipAttrs);
				  for(int i = 0; i < relationshipVals.length; i++){
					  mboSet.setQbe(relationshipAttrs[i], relationshipVals[i]);
				  }
...
// Comment out the following due to Issue # 1903 on project site				
	...
	public String createFollowup() {
		try{
			...
			mbo = ((PlusTWORemote)mbo).createWorkorder(); 
			id = mbo.getUniqueIDValue();
...
/*			for(MboRemote meter : mboList){
				if(!meter.getString("NEWREADING").isEmpty()){
					meter.setValue("INSPECTOR",	this.user.getUserId());

					}
			}*/
			
	/*		mbo.setValue("ACTIVEASSETMETER.NEWREADINGDATE", mbo.getDate(dtf.format(now)));
	...
	*/		
	...
	public String downtime() {
		try{			
			...
			if (dtRemote == null){
				populateMbo(OWNERMBO, this.getSessionValueByName(EMMConstants.CURRENTAPPNAME));
				id = mbo.getUniqueIDValue();
				PlusTWORemote wo = (PlusTWORemote)mbo;
	...
	@Action(value="starttimer",
			results={
				@Result(name="success", location="view.action", type="redirect", params={"id","${id}"}),
				//@Result(name="task", location="viewtask.action", type="redirect", params={"id","${id}"}),
				@Result(name="error", location="view.action", type="redirect", params={"id","${id}"})
			}
		)
	public String startTimer() {
		try {
			super.save();
			populateMbo(OWNERMBO,APPNAME);
			PlusTWORemote woRemote = (PlusTWORemote)mbo;
			woRemote.startTimer();
...
//			//redirect the view to stay on task page
//			if(id >= 0){
//				return "task";
//			}
		} catch (RemoteException e) {
...
//			
	...
	public String doselectOwner() {
		try{
            populateMbo(OWNERMBO, APPNAME);
            PlusTWORemote woRemote = (PlusTWORemote)this.mbo;
	...
	@Action(value="dochangestatus", results={
			@Result(name="success",location="${currentAction}",type="redirect"),
			@Result(name="error",location="${currentAction}",type="redirect")
		})	
	public String doChangeStatus(){
		...
		try {
			if (newStatus == null){
				this.setMessage(new EZMessage(mbo.getMessage("system","null",new String[]{this.mbo.getMboValueInfoStatic("STATUS").getTitle()}), EMMConstants.ERROR));
				return ERROR;
			}			
			...
			if((newStatus.equalsIgnoreCase("COMP") && !mbo.getString("STATUS").equalsIgnoreCase("WCOMP"))
					|| (newStatus.equalsIgnoreCase("WCOMP") && !mbo.getString("STATUS").equalsIgnoreCase("INPRG"))){
				setSessionObject(mboName, null);
				String msg = mbo.getMessage("aep", "priorstatus");
			...
			if(newStatus.equalsIgnoreCase("WCOMP") && !mbo.getMboSet("AEPWOACTIVITY").isEmpty()){				
				String msg = mbo.getMessage("aep", "AEPNoReasonforRepairchild");
				this.setMessage(new EZMessage(msg, EMMConstants.ERROR));
				return ERROR;		   
			}
			...
			if(newStatus.equalsIgnoreCase("WCOMP")){
	            ...
	            if(mbo.getInt("PLUSTPRIENTERED") == 0 && !assetMeterSet.isEmpty() &&  mbo.getInt("ISTASK") == 0){
		            setSessionObject(mboName, null);
					String msg = mbo.getMessage("aep", "Primary meter needs to be updated before changing status to WCOMP");
					this.setMessage(new EZMessage(msg, EMMConstants.ERROR));
					return ERROR;	                             
	            }
			}
			
			if (mbo instanceof StatefulMboRemote){
				((StatefulMboRemote)mbo).canChangeStatus(newStatus);
				((StatefulMboRemote)mbo).changeStatus(newStatus, this.user.getSession().getMXServerRemote().getDate(), memo);
			} else {
				mbo.setValue("STATUS", newStatus);
			}
			this.mbo.getThisMboSet().save();
			this.id = mbo.getUniqueIDValue();
			setSessionObject(mboName, null);
			this.setMessage(new EZMessage(mbo.getMessage("system","saverecord"), EMMConstants.SUCCESS));
		} catch (MXException e) {
			setSessionObject(mboName, null);
			String msg = MaximoExceptionUtil.getMessage(this.user.getSession(), e);
			this.setMessage(new EZMessage(msg, EMMConstants.ERROR));
