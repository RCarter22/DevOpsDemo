package com.interprosoft.ezmaxmobile.plusploc.action;
...
public class PluspLocAction extends BaseAction {
	private static final long serialVersionUID = 1L;
	
	public static final String OWNERMBO = "LOCATIONS";	
	public static final String APPNAME = "PLUSPLOC";
	...
	public String listchildren() {
		try {
			...
			populateMboListByRelationship("LOCHIERARCHY","CHILDREN");
		} catch (Exception e){
			this.setMessage(new EZMessage(e.getMessage(), EMMConstants.ERROR));
			this.addActionError(e.getMessage());
			return ERROR;	
	...
	public String listparents() {
		try {
			clearMboSession("CURRENTHIERARCHY");
			populateMbo("LOCHIERARCHY", APPNAME);
			MboRemote mboHierachy = this.simpleService.findById("LOCHIERARCHY",id);
			MboSetRemote parents = this.simpleService.getMboSet("LOCHIERARCHY");
			parents.setQbe("LOCATION", mboHierachy.getString("LOCATION"));
			parents.setQbe("SITEID", mboHierachy.getString("SITEID"));
			parents.setQbe("ORGID", mboHierachy.getString("ORGID"));
			parents.setQbe("SYSTEMID", mboHierachy.getString("SYSTEMID"));
			parents.setQbe("PARENT", "!=~null~");
			parents.setQbeExactMatch(true);
			mboList = new ArrayList<MboRemote>();
			MboRemote temp = parents.moveFirst();
			while(temp!=null)
			{
				mboList.add(temp.getMboSet("PARENTS").moveFirst());
				temp = parents.moveNext();
			}
		} catch (Exception e){
			this.setMessage(new EZMessage(e.getMessage(), EMMConstants.ERROR));
			this.addActionError(e.getMessage());
