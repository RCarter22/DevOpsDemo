...
@Component
@Scope("prototype")
@Namespace("/locationmeter")
@ResultPath(value="/")
@ParentPackage(value="mydefault")
@Results({@Result(name="error", location="/error/error.jsp")})
public class PluspLocMeterAction extends BaseAction {
	
	private static final long serialVersionUID = 1L;
	
	public static final String OWNERMBO = "LOCATIONMETER";
	...
	@Action(value="view",
			results={
				@Result(name="success", location="locationmeter.jsp"),
				@Result(name="error", location="locationmeter.jsp")
			}
		)
	public String view() {
		try{
			mbo = (MboRemote)this.getSessionObject("LOCATIONS"); 
			if(this.mbo== null && id>0){
				mbo = this.simpleService.findById("LOCATIONMETER",id);
			}else{
				mbo = this.simpleService.findById(mbo.getMboSet("LOCATIONMETER"), id);
			}
			setMboSession("LOCATIONMETER",this.mbo);
			// For IBM BUG!!!
			if(mbo.getString("METER.DOMAINID")!=null && !mbo.getString("METER.DOMAINID").equalsIgnoreCase(""))
			...
			if (mbo.getString("METER.METERTYPE").equals("CONTINUOUS")){			
				previousReadings = mbo.getMboSet("LOCMETERREADING");
