 ...
 * All rights reserved.
 ******************************************************************************/
package com.interprosoft.ezmaxmobile.plustassetcust.action;
...
@Component
@Scope("prototype")
@Namespace("/plustassetcust")
@ResultPath(value="/")
@ParentPackage(value="mydefault")
@Results({@Result(name="error", location="/error/error.jsp")})
public class PlusTAssetCustAction extends BaseAction {
	
	private static final long serialVersionUID = 1L;

	public static final String OWNERMBO = "ASSETUSERCUST";
	public static final String OWNERMBO2 = "AEPASSETDEPT";
	...
	public String execute() {
		try {
			...
			populateMboListByRelationship(currentMbo, "ASSETUSERCUST");
	...
	@Action(value="view",
			results={
				@Result(name="success", location="plustassetcust.jsp"),
				@Result(name="error", location="plustassetcust.jsp")
			}
		)
	public String view() {
		try{
	...
	public String delete() {
		try {
			...
			MboRemote mboRemote = this.simpleService.findById(mbo.getMboSet("ASSETUSERCUST"), id);
			id = tmpId;
			mboRemote.delete();
		} catch (Exception e){
			try{
	...
	@Action(value="setOtherRecordsAsNonDefault", results={
			@Result(name="success", type="redirect", location="view.action", params={"id","${id}"}),
			@Result(name="error", type="redirect", location="view.action", params={"id","${id}"})
		})
	public String setOtherRecordsAsNonDefault(){
		try {						
			String currentMbo = this.getSessionValueByName(EMMConstants.CURRENTMBONAME);
			if(currentMbo == null || currentMbo.equalsIgnoreCase(""))
				return EMMConstants.HOME;
			...
			if(mbo.isModified("ISUSER") && mbo.getBoolean("ISSER")){
				MboSetRemote aliasSet = mbo.getOwner().getMboSet("ASSETUSERCUST");
				MboRemote aliasMbo = aliasSet.moveTo(0);
				
				int counter = 1;
				while(aliasMbo != null){
					if (mbo.getUniqueIDValue() != aliasMbo.getUniqueIDValue()){ 						
						//aliasMbo.setValue("ISDEFAULT", false, 2L);						
					}
					System.out.println(aliasMbo.getString("DISPLAYNAME") + " " + aliasMbo.getString("ISUSER"));
					aliasMbo = aliasSet.moveTo(counter);
					counter++;
				}		
